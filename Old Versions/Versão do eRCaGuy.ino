...
void loop()
{
	...
	if (output_data == true) //if a pulse just came in
	{
		...
		//print values 
		Serial.print(F("pulse high(us) = ")); Serial.print(pulseTime);
		Serial.print(F("	pulse low(us) = ")); Serial.print(pd_us - pulseTime);
		Serial.print(F("	pulse time(us) = ")); Serial.print(pd_us, 1);
		Serial.print(F("		pulseFreq(Hz) = ")); Serial.print(pulseFreq, 1);
		Serial.print(F("	DutyCycle(%) = ")); Serial.print(100.0 * (pulseTime) / pd_us, 2);
		Serial.print(F("	PWM Value = ")); Serial.println(256.0 * (pulseTime) / pd_us, 2);
